...
- **Email/username normalization** at DB vs SMTP vs App layer mismatches -> Account collision (BrutSecurity #3016)
- **Rate limit** on OTP vs login vs password reset — often inconsistent (Bug0x #166)
- **Mobile UA** -> legacy API with weaker auth / different validation
...
- **Unicode email/username** (`victim@gmáil.com`) -> DB finds victim, SMTP sends to attacker (IDN homograph)
- **Rate limit bypass** on OTP/reset -> 0-click ATO chain (Bug0x #166)
- **postMessage** with decimal IP origin (`2130706433`) -> bypass origin check (BugXplorer #4082)
- **HTTP/2** instead of HTTP/1.1 -> WAF bypass (BugXplorer #4075)
- **CSS injection** instead of JS -> exfil tokens under strict CSP (BugXplorer #4088)
...
- **WAF response diff**: 403 on `UNION SELECT` but 200 on `UNiON SeLeCt` -> case variation bypass
- **postMessage listener** without strict origin check -> DOM XSS / ATO chain
- **CSS injection** surface (webmail, themes, rich text) -> exfil under CSP
- **Unicode normalization** mismatch (DB vs App vs SMTP) -> account collision
